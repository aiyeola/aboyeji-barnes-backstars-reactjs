...
export const requestTrip = (data) => async (dispatch) => {
  try {
    const response = await axios.post(
      `${BASE_URL}/api/v1/requests/${data.type}`,
      data.data,
      config
    );
    if (data.toggleAutofill) {
      await axios.patch('api/v1/auth/autofill-preference', {}, config);
    }
    dispatch({
...
export const updateRequest = (data) => async (dispatch) => {
  try {
    const response = await axios.put(
      `${BASE_URL}/api/v1/requests/${data.id}`,
      data.data,
      config
    );
    dispatch({
